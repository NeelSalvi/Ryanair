...
Then('I should see a text box for entering the email', async function () {
  ...
  await this.driver.wait(
    async () => await loginPage.getUserEmailTextbox(),
...
Then('I should see a text box for entering the password', async function () {
  const loginPage = new LoginPage(this.driver)
  await this.driver.wait(
    async () => await loginPage.getUserPasswordTextbox(),
    timeouts.STEP_TIMEOUTS.TIMEOUT
  )
})
...
And('I should see a sign in button', async function () {
  const loginPage = new LoginPage(this.driver)
  await this.driver.wait(
    async () => await loginPage.getSignInButton(),
    timeouts.STEP_TIMEOUTS.TIMEOUT
  )
})

And(
  'I should see a login header with text {string}',
...
And(
  'I should see a email address label with text {string}',
  async function (expectedText) {
    const loginPage = new LoginPage(this.driver)
...
And(
  'I attempt to login without any credentials',
  async function () {
    const loginPage = new LoginPage(this.driver)
